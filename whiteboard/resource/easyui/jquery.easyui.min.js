 ...
 * 
 * 修改内容：
 * function _195(_196): 第1982行，在click事件基础上添加touchstart事件
 * 第621行，在touchstart发生时模拟click事件，解决linkbutton无法点击的问题，并在panel层防止click冒泡
 * 第9355行添加timer检验，防止重复点击:
 *   _6e7.bind("touchstart.combo",function(){
 * 第9367行添加timer检验，防止某些系统同时触发click与touchstart导致重复点击:
 *   _6e7.bind("click.combo",function(){: 
 * 第10342行添加datebox的touch环境下click转换，并阻止该事件冒泡。
 * 第10940行添加一个定时器，定时纠正window-shadow的位置。
 * 第10407行日期控件不可写，第5489行防止日期重复点击
 * 10437:datetimebox为ok按钮添加e.preventDefault();e.stopPropagation();
 * 10345&10348:datebox为close和current按钮添加e.preventDefault();e.stopPropagation();
 * 1016:通过“$.easyTreeShowCheckLoading”来控制哪些带checkbox的tree需要显示防止重复点击的Mask
 */
(function($){
...
if(!$.data(this,"droppable")) return false;
...
var iconConvert = ["icon-ok", "icon-cancel"];
for (var i=0;i<iconConvert.length;i++){
	if (_61.iconCls == iconConvert[i]){
		_61.iconCls = "jl-"+iconConvert[i].substring(5, iconConvert[i].length);
	}
}

if (_61.iconCls && _61.iconCls.indexOf("jl-") == 0){
	var okBtn = $("<input type='button' value='' class='jl-button "+_61.iconCls+"'>");
	okBtn.attr("id",_61.id);
	var clickEvent = _60.onclick;
	if (_61.text.indexOf("<!") != -1){
		okBtn.val(_61.text.substring(0,_61.text.indexOf("<!")));
	}
	else{
		okBtn.val(_61.text);
	}
	
	$(_60).replaceWith(okBtn);
	$.data(okBtn,"linkbutton", {options:_61});
	if (clickEvent){
		okBtn.click(function(){
			clickEvent.call(_5f);
		});
		if("ontouchstart" in window) {
			okBtn.bind("touchstart", function(){
				okBtn.trigger("click");
			});
		}
	}
	linkBtnAble(okBtn, _61.disabled);
}else{
	$(_60).empty();
	$(_60).addClass("l-btn").addClass("pulb-btn");
	if(_61.id){
	$(_60).attr("id",_61.id);
	}else{
	$(_60).attr("id","");
...
function linkBtnAble(target, disable){
var targetCls = $(target).attr("class");
if (targetCls.indexOf("button") != -1 &&
		targetCls.indexOf("jl-") != -1){
	if (disable){
...
_5f(this);
var onlyItem = $(this).first();
onlyItem.unbind("touchstart").bind("touchstart",function(e){onlyItem.trigger("click");});
$(this).closest(".messager-body").unbind("touchstart click").bind("touchstart click", function(e){e.stopPropagation();e.preventDefault();return false;});
});
};
...
return jq.each(function(){
_62(this,false);
linkBtnAble(this,false);
});
},disable:function(jq){
return jq.each(function(){
_62(this,true);
linkBtnAble(this,true);
});
}};
...
$.fn.pagination.defaults={total:1,pageSize:10,pageNumber:1,pageList:[10,15],loading:false,buttons:null,showPageList:true,showRefresh:true,onSelectPage:function(_8b,_8c){
...
}).bind("click.tree touchstart.tree",function(){
var _this = this;
var mocb=getTop().ICApp.Status.maskOnClickButton;
mocb(function(){
	_144(_9e,_this);
...
var generateTimeoutMask = function(com, timeout) {
	if (!timeout) {
		timeout = 500;
	}
	var options = {
		position : "absolute",
		left : $(com).offset().left,
		top : $(com).offset().top,
		width : $(com).width(),
		height : $(com).height(),
		zIndex : "9999",
	...
	var comMask = $("<div></div>").css(options).appendTo(document.body);
	setTimeout(function() {
		comMask.remove();
	}, timeout);
};
if($.easyTreeShowCheckLoading){
generateTimeoutMask($(_c1).closest(".easyui-tree"));	
}
function _c6(_c9){
...
if(opts.collapsible){
$("<a class=\"panel-tool-collapse\" href=\"javascript:void(0)\"></a>").appendTo(tool).bind("click touchstart",function(){
...
if(opts.minimizable){
$("<a class=\"panel-tool-min\" href=\"javascript:void(0)\"></a>").appendTo(tool).bind("click touchstart",function(){
_1b8(_196);
return false;
});
}
if(opts.maximizable){
$("<a class=\"panel-tool-max\" href=\"javascript:void(0)\"></a>").appendTo(tool).bind("click touchstart",function(){
...
if(opts.closable){
$("<a class=\"panel-tool-close\" href=\"javascript:void(0)\"></a>").appendTo(tool).bind("click touchstart",function(){
_199(_196);
return false;
});
}
...
if(!_1ea || !_1ea.proxy) return false;
...
if(!window.isLenovoPad){
var av=(navigator.appVersion+"").toLowerCase();
window.isLenovoPad=(av.indexOf("windows nt 6.2") > -1)&&("ontouchstart" in window);
}
if(window.isLenovoPad){
$.fn.window.defaults=$.extend({},$.fn.panel.defaults,{zIndex:9000,draggable:false,resizable:false,shadow:true,modal:false,inline:false,title:"New Window",collapsible:true,minimizable:true,maximizable:true,closable:true,closed:false});
}else{
...
var alinkBtn = $("<a></a>").attr("href","javascript:void(0)").text(_215).css("margin-left",10);
if (typeof _214[_215] == "function"){
	alinkBtn[0].onclick = _214[_215];
}
else{
	eval("alinkBtn[0].onclick = "+_214[_215]);
}
alinkBtn.appendTo(tb).linkbutton({iconCls:"jl-default"});
}
}
...
$(".window").filter(function(){var positionOK = (this.style.bottom=="0px")&&(this.style.right=="0px");
var contentOK = ($(this).find(".messager-body").html() == opts.msg);var titleOK = ($(this).find(".panel-title").html() == opts.title);
return (titleOK && contentOK && positionOK);}).find("a.panel-tool-close").trigger("click");
...
$(_3c7).find(".calendar-nextmonth").bind("click touchstart", function(){
window.calendarNextMonth=function(){_3c8(_3c7,1);};
getTop().ICApp.Status.canNotDoubleClickButton(window.calendarNextMonth, "calendarNextMonth", window, 800);
});
$(_3c7).find(".calendar-prevmonth").bind("click touchstart", function(){
window.calendarPrevMonth=function(){_3c8(_3c7,-1);};
getTop().ICApp.Status.canNotDoubleClickButton(window.calendarPrevMonth, "calendarPrevMonth", window, 800);
});
$(_3c7).find(".calendar-nextyear").bind("click touchstart", function(){
window.calendarNextYear=function(){_3cb(_3c7,1);};
getTop().ICApp.Status.canNotDoubleClickButton(window.calendarNextYear, "calendarNextYear", window, 800);
});
$(_3c7).find(".calendar-prevyear").bind("click touchstart", function(){
window.calendarPrevYear=function(){_3cb(_3c7,-1);};
getTop().ICApp.Status.canNotDoubleClickButton(window.calendarPrevYear, "calendarPrevYear", window, 800);
});
...
_6e7.bind("touchstart.combo",function(){
//getTop().ICApp.Status.myDistinctShow("click combo-arrow");
var combof = $(this).closest(".combo").prev(".combo-f");
if(!getTop().ICApp.Status.canCallComboArrowClick(combof)) return;
if(_6e5.is(":visible")){
_6ef(_6e2);
}else{
...
_791.find(".datebox-current").click(function(e){
e.preventDefault();e.stopPropagation();
_78d.calendar.calendar({year:new Date().getFullYear(),month:new Date().getMonth()+1,current:new Date()});
});
_791.find(".datebox-close").click(function(e){
e.preventDefault();e.stopPropagation();
$(_78c).combo("hidePanel");
});
var cantouchclick = function(o) {
$(o).unbind("touchstart.safecalendar").bind("touchstart.safecalendar", function(e) {
if(e.target.className == "calendar-day"){var s = $(e.target).attr("abbr");
$(o).find(".calendar-selected").removeClass("calendar-selected");
$(e.target).addClass("calendar-selected");
var ds = s.split(",");var date = new Date(ds[0], parseInt(ds[1]) - 1, ds[2]);
var _790 = opts.formatter(date);_794(_78c, _790);opts.onSelect.call(_78c, date);
}else{$(e.target).trigger("click");}});};
cantouchclick(_78f);
};
};
...
$.fn.datebox.defaults=$.extend({},$.fn.combo.defaults,{editable:false,panelWidth:180,panelHeight:"auto",keyHandler:{up:function(){
},down:function(){
},enter:function(){
_795(this);
},query:function(q){
...
//if ('ontouchstart' in window) {
//扩展tree，选中结点时展开/收缩子结点，可针对平板优化
$.fn.tree.defaults = $.extend({},$.fn.tree.defaults,{onSelect:function(node){
    var isLeaf = $(this).tree('isLeaf', node.target);
    if (!isLeaf) {
        var isExpanded = $(node.target).children("span.tree-hit").hasClass("tree-expanded");
        if (!isExpanded){
...
$.fn.combotree.defaults = $.extend({},$.fn.combotree.defaults,{onSelect:function(node){
	var isExpanded = $(node.target).children("span.tree-hit").hasClass("tree-expanded");
    if (!isExpanded){
    	$(this).tree('expand',node.target);
    }
...
if (navigator.userAgent.indexOf("Windows NT") == -1) {
	window.onload=function(){
		...
		for (var i=0;i<linkTags.length; i++){
	    	if (linkTags[i].getAttribute("href").indexOf("easyui.css") != -1 ){
	    		linkTags[i].disable = true;
	    		var cssPath = linkTags[i].getAttribute("href").split("easyui.css")[0];
	    		var newLinkTag = document.createElement("link");
	    		newLinkTag.setAttribute("type", "text/css");
	    		newLinkTag.setAttribute("rel", "stylesheet");
	    		newLinkTag.setAttribute("media", "screen");
	    		newLinkTag.setAttribute("title", "styles1");
	    		newLinkTag.setAttribute("href", cssPath+"easyui_t.css?a=1");
	    		document.body.appendChild(newLinkTag);
	    		newLinkTag.disable = false;
	    		break;
	    	}
	    }
	}
	
	//判断window对象中是否存在onorientationchange事件
	...
	window.addEventListener(orientationEvent, function() {
	    ...
	    //iPad和Android操作系统的横竖屏判断不一致
	    if ((ua.indexOf("iPad") > 0 && Math.abs(window.orientation) != 90)
	    		|| (ua.indexOf("Android") > 0 && screen.width<screen.height)) {
	    	//如果是竖屏，则需要提醒用户系统不支持竖屏
	    	if (!document.getElementById("allScreenDiv"))
	    	{
	    		var sWidth=document.body.offsetWidth;
	            var sHeight=screen.height;
	            var bgObj=document.createElement("div");
	            bgObj.setAttribute('id','allScreenDiv');
	            ...
	            bgObj.style.zIndex = "999999";
	            document.body.appendChild(bgObj);
	            
	            sWidth = sWidth/2-100;
	    	    sHeight = sHeight/2-100;
	    	    
	    	    $("<div></div>").html("系统不支持竖屏操作！")
	    	    .css({"margin-left":sWidth,"margin-top":sHeight,
	    	    	"background-color":"#fff","width":200,"height":50})
	    	    .appendTo(document.getElementById("allScreenDiv"));
	    	}
	    }
	    else{
	    	var allScreenDiv = document.getElementById("allScreenDiv");
	    	if (allScreenDiv){
	    		document.body.removeChild(allScreenDiv);
	    	}
	    }
	}, false);
}
...
// check position of easyUI window and shadow, function getTop is used for get top window while getIcApp() sends error
// var getTop = function(w){if(!w){return getTop(window);}else{if(w == w.parent){return w;}else{return getTop(w.parent);}}};
window.setInterval(function(){
	var curTop = getTop();
	if(curTop && curTop.ICApp && curTop.ICApp.Status){
		curTop.ICApp.Status.checkShadow();
		curTop.ICApp.Status.checkBody();
	}
}, 1000);

var loadRussianFonts = function(){
	if(getTop() && getTop().ICApp && "ru" == getTop().ICApp.Status.getUILan()){
		var style = document.createElement("style");
		style.type = "text/css";
		style.innerHTML = "*{font-family:Arial, Times New Roman;font-size:9pt;}";
		document.body.appendChild(style);
	}
};
function checkNoMsgbox(callback) {
	var allowCallback = false;
	var mb = $(".messager-body");
	if (mb.size() == 0) {
		allowCallback = true;
	} else {
		var opts = mb.data("panel").options;
		var panel = mb.data("panel").panel;
		if (opts.onBeforeClose.call(mb) == false) {
			allowCallback = true;
		} else {
			panel.hide();
			opts.closed = true;
			opts.onClose.call(mb);
		}
	}
