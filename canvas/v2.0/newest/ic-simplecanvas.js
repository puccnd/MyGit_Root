...
(function($) {
	$.getTexture = function(withBase64Fn, jcObject) {
		if (!jcObject) {
			if (!window.jc)
				return;
			jcObject = window.jc;
		}
		var objs = jcObject.objects.slice(0);
		var minx = null, miny = null, maxx = null, maxy = null;
		for ( var i = 0; i < objs.length; i++) {
			var rect = objs[i].rect;
			if (objs[i].type == "line") {
				rect.left -= objs[i].context.lineWidth / 2;
				rect.top -= objs[i].context.lineWidth / 2;
				rect.width += objs[i].context.lineWidth;
				rect.height += objs[i].context.lineWidth;
			}
			var center = objs[i].center;
			var sita = objs[i].transform.rotate;
			if (sita == 0) {
				if (minx == null)
					minx = rect.left;
				else
				...
				var pos1 = $.PointInShapeJudger.getRoatatedXY(rect.left,
						rect.top, center.x, center.y, sita);
				var pos2 = $.PointInShapeJudger.getRoatatedXY(rect.left,
						rect.top + rect.height, center.x, center.y, sita);
				var pos3 = $.PointInShapeJudger.getRoatatedXY(rect.left
						+ rect.width, rect.top, center.x, center.y, sita);
				var pos4 = $.PointInShapeJudger.getRoatatedXY(rect.left
						+ rect.width, rect.top + rect.height, center.x,
						center.y, sita);
				if (minx == null)
					minx = Math.min(pos1.x, pos2.x, pos3.x, pos4.x);
				else
					minx = Math.min(minx, pos1.x, pos2.x, pos3.x, pos4.x);
				if (miny == null)
					miny = Math.min(pos1.y, pos2.y, pos3.y, pos4.y);
				else
					miny = Math.min(miny, pos1.y, pos2.y, pos3.y, pos4.y);
				if (maxx == null)
					maxx = Math.max(pos1.x, pos2.x, pos3.x, pos4.x);
				else
					maxx = Math.max(maxx, pos1.x, pos2.x, pos3.x, pos4.x);
				if (maxy == null)
					maxy = Math.max(pos1.y, pos2.y, pos3.y, pos4.y);
				else
					maxy = Math.max(maxy, pos1.y, pos2.y, pos3.y, pos4.y);
			}
		}
		...
		jcObject.clipImages({
			...
			if (withBase64Fn)
				withBase64Fn.call(null, base64);
		});
	};
	$.modifyJcBackground = function(url, jcObject) {
		jcObject = jcObject || window.jc;
		if (!jcObject) {
			jlShow("画板还未初始化！");
			return;
		}
		var render = jcObject.drawMaterials.render[0];
		render.style.backgroundImage = "url(" + url + ")";
	};
	$.drawImageIntoJc = function(jcObject, imageUrl) {
		jcObject.addImage(imageUrl);
		jcObject.redrawAll();
	};
	var newDiv = "<div />";
	...
	$.fn.openTopic = function(options) {
		var topicTitle = options.topicTitle, topicImage = options.topicImage;
		...
		var topic2 = $('<canvas id="re' + randomId
				+ '" class="idle-canvas"></canvas>');
		var topic3 = $('<canvas id="' + randomId + '" redrawAt="re' + randomId
				+ '" class="idle-canvas"></canvas>');
	...
	var simpleCanvas = function(object, options) {
		var instance = this;
		instance.clipImages = function(from, to, callback) {
			var getTheSelectedObject = function() {
				if ($("." + selectedCls).size() > 0) {
					var selectedCanvas = $("." + selectedCls).find("canvas")[0];
					var transform = $(".jc-select")[0].style.transform;
					var deg = transform.replace(/^.*rotate\(|deg\).*$/g, "")
							* Math.PI / 180;
					return {
			...
			var getDivRect = function(div) {
				return $(div)[0].getBoundingClientRect();
			};
			var can = instance.drawMaterials.render;
			var simwh = {
				width : can.width(),
				height : can.height()
			};
			...
			var simCanvas = $("<canvas></canvas>").attr(simwh).css(simwh)
					.appendTo("body").addClass("jc-tmp-canvas");
			var tmpCanvas = $("<canvas></canvas>").attr(size).css(size)
					.appendTo("body").addClass("jc-tmp-canvas");
			var bg = $.getBgImg(can);
			if (!bg) {
				var tmpCtx = tmpCanvas[0].getContext("2d");
				var selected = getTheSelectedObject();
				if (!selected) {
					var simCtx = simCanvas[0].getContext("2d");
					simCtx.drawImage(can[0], 0, 0, simwh.width, simwh.height);
					var data = simCtx.getImageData(from.x, from.y, to.x
							- from.x, to.y - from.y);
					tmpCtx.putImageData(data, 0, 0, 0, 0, to.x - from.x, to.y
							- from.y);
					var dataURL = tmpCanvas[0].toDataURL();
					callback.call(null, dataURL);
					$(".jc-tmp-canvas").remove();
				} else {
					var simCtx = simCanvas[0].getContext("2d");
					var rect = getDivRect($("." + selectedCls));
					var center = {
					...
					simCtx.drawImage(can[0], 0, 0, simwh.width, simwh.height);
					simCtx.save();
					simCtx.translate(center.x, center.y);
					simCtx.rotate(selected.deg);
					var cwidth = $(selected.canvas).width();
					var cleft = -cwidth / 2;
					var cheight = $(selected.canvas).height();
					var ctop = -cheight / 2;
					simCtx.drawImage(selected.canvas, cleft, ctop, cwidth,
							cheight);
					simCtx.restore();
					var data = simCtx.getImageData(from.x, from.y, to.x
							- from.x, to.y - from.y);
				...
				img.onload = function() {
					...
					$(this).css(targetWH);
					var simCtx = simCanvas[0].getContext("2d");
					simCtx.drawImage(this, (cw - targetWH.width) / 2,
							(ch - targetWH.height) / 2, targetWH.width,
							targetWH.height);
					simCtx.drawImage(can[0], 0, 0, cw, ch);
					var tmpCtx = tmpCanvas[0].getContext("2d");
					var selected = getTheSelectedObject();
					if (!selected) {
						var data = simCtx.getImageData(from.x, from.y, to.x
								- from.x, to.y - from.y);
						tmpCtx.putImageData(data, 0, 0, 0, 0, to.x - from.x,
								to.y - from.y);
						var dataURL = tmpCanvas[0].toDataURL();
						callback.call(null, dataURL);
						$(".jc-tmp-canvas").remove();
		...
		instance.initContext = function() {
			...
			ctx.strokeStyle = ctx.fillStyle = (options ? options.color : "red")
					|| "red";
		...
		instance.addImage = function(url) {
			...
			getNewImage(url).afterLoaded(
					function() {
						var img = this, iw = img.width, ih = img.height;
		...
		var drawRendObj = function(ctx, object) {
			if (object.type == "line") {
				...
				if (object.data.length > 1) {
					if (withRotate) {
						ctx.moveTo(from.x - object.center.x, from.y
								- object.center.y);
					} else {
					...
					for ( var i = 1; i < object.data.length - 1; i++) {
						...
						if (withRotate) {
							ctx.lineTo(to.x - object.center.x, to.y
									- object.center.y);
						} else {
					...
					if (instance.topicMode != 2) {
						if (withRotate) {
							ctx.arc(from.x - object.center.x, from.y
									- object.center.y,
		...
		var drawRendImg = function(ctx, object) {
			if (object.type == "img") {
				var rect = object.rect;
				var withRotate = (object.transform.rotate != 0);
				getNewImage(object.data)
						.afterLoaded(
								function() {
									ctx.save();
									$(this).attr({
									...
									ctx.beginPath();
									if (withRotate) {
										ctx.translate(object.center.x,
												object.center.y);
										ctx.rotate(object.transform.rotate);
										ctx.drawImage(this, rect.left
												- object.center.x, rect.top
												- object.center.y, rect.width,
												rect.height);
									} else {
										ctx.drawImage(this, rect.left,
												rect.top, rect.width,
		...
		var drawRendObj2 = function(ctx, object) {
		...
		var drawSelected = function(div, object) {
			...
			var rect = object.rect, stl = absStyle(rect);
			...
			div.css(stl).append(can).appendTo("body");
			...
			var dragOpts = {
				...
				setObjectParams : function(startParams, endParams) {
					...
					if (startParams.l + 0.5 * startParams.w != endParams.l
							+ 0.5 * endParams.w) {
						object.transform.translateX = parseInt((endParams.l + 0.5 * endParams.w)
								- (startParams.l + 0.5 * startParams.w));
					}
					if (startParams.t + 0.5 * startParams.h != endParams.t
							+ 0.5 * endParams.h) {
						var b = parseInt((endParams.t + 0.5 * endParams.h)
								- (startParams.t + 0.5 * startParams.h));
				...
				getNewImage(object.data).afterLoaded(
						function() {
							$(this).attr({
								...
								height : object.rect.width,
							});
							ctx.drawImage(this, 0, 0, object.rect.width,
									object.rect.height);
						});
			...
			for ( var i = 0; i < instance.objects.length; i++) {
				...
				if (o.selected) {
					...
					if (o.type == "line") {
						...
						drawRendImg(dm.renderContext, o);
					} else {
						drawRendObj2(dm.renderContext, o);
					}
				}
			}
		};
		...
		var PointInShapeJudger = $.PointInShapeJudger = {
		...
		instance.selectObject = function(pos) {
			var sel = false, tmpSelected = instance.selected;
			for ( var i = 0; i < instance.objects.length; i++) {
		...
		instance.transObject = function(object) {
			if (object.type == "line") {
				...
				for ( var index in dataArr) {
					dataArr[index] = {
						x : (dataArr[index].x + tx - object.center.x) * sx
								+ object.center.x,
						y : (dataArr[index].y + ty - object.center.y) * sy
								+ object.center.y
					};
				}
	...
	$.fn.pulbDraggable = function(options) {
		...
		var doMultiTransform = function(dom, helper, pos, rateLimit) {
			...
			var stEvtAngle = Math.atan2(stPos.y2 - stPos.y1, stPos.x2
					- stPos.x1);
			var evtAngle = Math.atan2(pos.y2 - pos.y1, pos.x2 - pos.x1);
			var stDis = Math.sqrt((stPos.x1 - stPos.x2) * (stPos.x1 - stPos.x2)
					+ (stPos.y1 - stPos.y2) * (stPos.y1 - stPos.y2));
			var dis = Math.sqrt((pos.x1 - pos.x2) * (pos.x1 - pos.x2)
					+ (pos.y1 - pos.y2) * (pos.y1 - pos.y2));
			var rate = dis / stDis;
			if (rateLimit) {
			...
			var newA = helper.startAngle + (180 / Math.PI)
					* (evtAngle - stEvtAngle);
			$(dom).css({
				left : newL,
				top : newT,
	...
	var getBtnStr = function(id, dir, stl) {
		return $("<div class='operateBtn' id='" + id
				+ "' style='background:url(../resource/images/arrow_" + dir
				+ ".png);" + stl + "'></div>");
	};
	$.fn.pulbMouseTranform = function(options) {
		...
		var relScale = function(a, b, o) {
			var dis1 = Math.sqrt((a.x - o.x) * (a.x - o.x) + (a.y - o.y)
					* (a.y - o.y));
			var dis2 = Math.sqrt((b.x - o.x) * (b.x - o.x) + (b.y - o.y)
					* (b.y - o.y));
			return dis2 / dis1;
		};
		var $document = $(document);
	...
	$(function() {
		if ($("#simThread").size() == 0) {
			var ifm = "<iframe id='simThread' src='../deskApp/thread.jsp'></iframe>";
			$(ifm).hide().appendTo("body");
		}
		$.startScriptThread("../resource/canvas/ic-thread.js", false);
	});
})(jQuery);
